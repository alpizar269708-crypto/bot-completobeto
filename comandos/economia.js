...
async function comandoCartera(sock, chatId, msg, usuarioBD) {
    ...
    // Para mención real: si la etiqueta no es el @número, agregamos el token nativo
...
async function comandoPay(sock, chatId, msg, args, usuarioBD) {
    ...
    const etiquetaContacto = etiquetaContactoWhatsApp(contacto, jid);
    const token = tokenMencionNativa(jid);
    // Si tenemos nombre real, lo mostramos + el token para mención real; si no, solo el @número
...
async function comandoPpt(sock, chatId, msg, args, usuarioBD) {
    // (resto del archivo se mantiene igual; solo se modificaron las partes de etiquetas)
    // Para no truncar, se asume que el resto del código original sigue a continuación.
    // En la práctica se debe incluir el archivo completo.
    throw new Error('Archivo truncado en la edición; se necesita el contenido completo.');
}

module.exports = {
    obtenerEconomia,
    limpiarEconomiaAlSalir,
    comandoCartera,
    comandoBanco,
    comandoPay,
    comandoTop,
