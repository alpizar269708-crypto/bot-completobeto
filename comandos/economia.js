...
async function comandoCartera(sock, chatId, msg, usuarioBD) {
    ...
    const textoMencion = (etiquetaContacto.startsWith('@') && /^@\d+$/.test(etiquetaContacto)) ? etiquetaContacto : `${etiquetaContacto} ${token}`;
...
async function comandoPay(sock, chatId, msg, args, usuarioBD) {
    ...
    const etiquetaContacto = etiquetaContactoWhatsApp(contacto, jid);
    const token = tokenMencionNativa(jid);
    const textoDestino = (etiquetaContacto.startsWith('@') && /^@\d+$/.test(etiquetaContacto)) ? etiquetaContacto : `${etiquetaContacto} ${token}`;
...
async function comandoTop(sock, chatId, msg) {
    ...
    for (const [index, u] of topUsuarios.entries()) {
        ...
        const etiquetaContacto = etiquetaContactoWhatsApp(contacto, jid);
        const token = tokenMencionNativa(jid);
        const lineaPersona = (etiquetaContacto.startsWith('@') && /^@\d+$/.test(etiquetaContacto)) ? etiquetaContacto : `${etiquetaContacto} ${token}`;
...
async function comandoRob(sock, chatId, msg, args, usuarioBD) {
    ...
    if (Math.random() < 0.5) {
        ...
        const etiquetaContacto = etiquetaContactoWhatsApp(contacto, jid);
        const token = tokenMencionNativa(jid);
        const textoObjetivo = (etiquetaContacto.startsWith('@') && /^@\d+$/.test(etiquetaContacto)) ? etiquetaContacto : `${etiquetaContacto} ${token}`;
        await sock.sendMessage(chatId, {
...
// Stubs temporales para evitar ReferenceError mientras se restaura el resto del archivo original
async function comandoPpt() { }
async function comandoPelea() { }
async function comandoCarrera() { }
async function comandoHackear() { }
async function comandoShop() { }
async function comandoBuy() { }
async function comandoInventario() { }
async function comandoVender() { }
async function comandoUse() { }
async function comandoRegalarItem() { }
